...
import { signReceiptTokenAction } from '../[id]/_lib/share-actions'
...
export function ReceiptPdfActions({ receiptId, number, totalAmount, clientEmail }: Props) {
  ...
  async function getOrGeneratePublicLink(): Promise<string> {
    ...
    const { token } = await signReceiptTokenAction(receiptId)
  ...
  function buildMessage(link: string): string {
    ...
    return `Hola, te envío el recibo ${number} por un importe de ${total} €.\n\nPuedes verlo online y descargar el PDF aquí:\n${link}\n\nGracias por tu confianza.`
  }

  function handleDownload() {
    window.open(`/api/recibos/${receiptId}/pdf`, '_blank')
  }
  ...
  async function handleSendEmail() {
    ...
    try {
      const res = await fetch(`/api/recibos/${receiptId}/email`, { method: 'POST' })
      if (!res.ok) {
  ...
  function handleEmailClient() {
    ...
    startTransition(async () => {
      try {
        ...
        const subject = encodeURIComponent(`Recibo ${number}`)
        const body = encodeURIComponent(
          `Hola, te envío el recibo ${number} por un importe de ${total} €.\n\nPuedes verlo online y descargar el PDF aquí:\n${link}\n\nGracias por tu confianza.`,
        )
