...
import { RECEIPT_STATUS_LABELS } from '../_lib/receipt-status'
...
export function ReceiptSearch({ initialSearch, initialStatus, clientId }: ReceiptSearchProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(() => {
      ...
      router.push(`/recibos${query ? `?${query}` : ''}`)
    })
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-2">
      ...
      >
        ...
        {(Object.entries(RECEIPT_STATUS_LABELS) as [string, string][]).map(([value, label]) => (
