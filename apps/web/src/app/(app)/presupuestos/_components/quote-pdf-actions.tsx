...
import { signQuoteTokenAction } from '../[id]/_lib/share-actions'
...
export function QuotePdfActions({ quoteId, number, totalAmount, clientEmail }: Props) {
  ...
  async function getOrGeneratePublicLink(): Promise<string> {
    ...
    const { token } = await signQuoteTokenAction(quoteId)
  ...
  function buildMessage(link: string): string {
    ...
    return `Hola, te envío el presupuesto ${number} por un importe de ${total} €.\n\nPuedes verlo online y descargar el PDF aquí:\n${link}\n\nGracias por tu confianza.`
  }

  function handleDownload() {
    window.open(`/api/presupuestos/${quoteId}/pdf`, '_blank')
  }
  ...
  async function handleSendEmail() {
    ...
    try {
      const res = await fetch(`/api/presupuestos/${quoteId}/email`, { method: 'POST' })
      if (!res.ok) {
        const body = await res.json().catch(() => ({ error: 'Error al enviar el email' }))
  ...
  function handleEmailClient() {
    ...
    startTransition(async () => {
      try {
        ...
        const subject = encodeURIComponent(`Presupuesto ${number}`)
        const body = encodeURIComponent(
          `Hola, te envío el presupuesto ${number} por un importe de ${total} €.\n\nPuedes verlo online y descargar el PDF aquí:\n${link}\n\nGracias por tu confianza.`,
        )
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          <MailIcon />
          {loadingAction === 'email' ? 'Abriendo…' : 'Email cliente'}
        </button>
