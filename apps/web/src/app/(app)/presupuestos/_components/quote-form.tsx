...
import { createQuoteDraft } from '../_lib/quote-actions'
...
export function QuoteForm() {
  ...
  const [validUntil, setValidUntil] = useState(in30DaysISO())
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await createQuoteDraft(payload)
      if (res.ok) {
    ...
    <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-6">
        ...
        <section className="p-4 bg-[var(--surface)] border border-[var(--border)] rounded-lg space-y-4">
          <h2 className="text-lg font-medium text-[var(--text)]">Datos del presupuesto</h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              />
              {errors.validUntil && (
                <p className="text-xs text-[var(--danger)] mt-1">{errors.validUntil[0]}</p>
              )}
            </div>
          </div>
        </section>
