...
import { ShareButton, WhatsAppShareButton } from './_components/receipt-actions'
...
export default async function PublicReceiptPage({ params }: Props) {
  ...
  const vatBreakdown = receipt.lines.reduce(
    (acc, line) => {
  ...
  const shareTitle = `Recibo ${receipt.number} — ${tenant.name}`
    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      <div className="max-w-lg mx-auto min-h-screen flex flex-col">
        <header className="px-5 pt-6 pb-4 flex items-center justify-between">
          ...
          <ShareButton url={`/r/${token}`} title={shareTitle} />
        </header>
          ...
          <section className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6">
            ...
            <div className="border-t border-[var(--border)] mt-5 pt-4 space-y-2">
              ...
              <div className="flex justify-between text-sm pt-1">
                ...
                <span className="text-[var(--text)]">
                  {formatCurrency(Number(receipt.subtotal))}
                </span>
              </div>
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    issued: 'bg-[var(--success)]/10 text-[var(--success)]',
